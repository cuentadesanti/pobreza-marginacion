...
def build(Y, ind, K, rural, X, state=None, free=False):
    N, J = Y.shape
    IDX = {n: i for i, n in enumerate(ind)}
    facs = ["material", "educativo", "monetario"]
    anchor_rows = [IDX[gl.ANCHOR[f]] for f in facs]
    mask = np.ones((J, K))
...
def main():
    ...
    mod, facs = build(Y, ind, 3, rural, X, state if a.rung == 3 else None, free=a.free)
    with mod:
